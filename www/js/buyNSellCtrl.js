...
.controller('BuyNSellCtrl', function($scope,$ionicModal,$http,$rootScope,$ionicFilterBar,Upload,$ionicPopup,ionicToast,$ionicLoading) {
	...
	$scope.showItemDetails = function (id){
		...
		for (var i = 0; i <= $rootScope.products.Table.length ; i++) {
			if($rootScope.products.Table[i].BuySellID == id) {
				$scope.currentProduct = $rootScope.products.Table[i];
				$scope.hideWaiter();
			}
		}
	...
	$scope.getOther =function ()
	{
		...
		function successCallback(response) {
			...
			$rootScope.products =JSON.parse(jsonResponse.string.__text);
			console.log($rootScope.products.Table);
			$scope.hideWaiter();
		}
