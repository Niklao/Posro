angular.module('starter.servicesCtrl', ['ui.router'])

.controller('ServicesCtrl', function($scope,$ionicModal,$http,$rootScope,$ionicFilterBar,Upload,$ionicPopup,ionicToast,$ionicLoading,loadManager) {
		
	$scope.image ='data:image/png;base64,';
	
	$rootScope.services;
	$scope.currentService;
	
	/////////////////////////////////////////////////////////////////////////////////////////////////
	
	$ionicModal.fromTemplateUrl('templates/serviceDetails.html', {
		scope: $scope
	}).then(function(modal) {
		$scope.serviceDetailModal = modal;
	});

	$scope.showServiceDetails = function (id){
		loadManager.showWaiter();
		$scope.serviceDetailModal.show();
		for (var i = 0; i < $rootScope.services.Table.length ; i++) {
			if($rootScope.services.Table[i].SPID == id) {
				$scope.currentService = $rootScope.services.Table[i];
				loadManager.hideWaiter();
			}
		}
		
	};
	
	$scope.closeServiceDetails = function (){
		$scope.serviceDetailModal.hide();
	};

	/////////////////////////////////////////////////////////////////////////////////////////////////
	
	$scope.getOther =function ()
	{
		loadManager.showWaiter();
		$http({method: 'POST',url: urlBase+'/GetOthers',data: $.param({CategoryID: '0',StoreTypeID: '200'}),headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(
		function successCallback(response) {
			var x2js = new X2JS();
			jsonResponse = x2js.xml_str2json(response.data);
			$rootScope.services =JSON.parse(jsonResponse.string.__text);
			loadManager.hideWaiter();
		}
		, function errorCallback(response) {ionicToast.show('Load Failed.','middle',false,2500);loadManager.hideWaiter();});
	};
	
	$scope.getCategoryForStoreType =function ()
	{
		$http({method: 'POST',url: urlBase+'/GetCategoryForStoreType',data: $.param({StoreTypeID: '200'}),headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(
		function successCallback(response) {
			var x2js = new X2JS();
			var jsonResponse;
			jsonResponse = x2js.xml_str2json(response.data);
			$scope.jsonResponse = JSON.parse(jsonResponse.string.__text);
			loadManager.hideWaiter();
		}
		, function errorCallback(response) {ionicToast.show('Load Failed.','middle',false,2500);});
	};
	
	$scope.getServicesForCategory = function (id){
		loadManager.showWaiter();
		$http({method: 'POST',url: urlBase+'/GetOthers',data: $.param({CategoryID: id,StoreTypeID: '200'}),headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(
		function successCallback(response) {
			var x2js = new X2JS();
			jsonResponse = x2js.xml_str2json(response.data);
			$rootScope.services =JSON.parse(jsonResponse.string.__text);
			console.log($rootScope.services.Table);
			loadManager.hideWaiter();
		}
		, function errorCallback(response) {ionicToast.show('Load Failed.','middle',false,2500);loadManager.hideWaiter();});
	};
	
	
	/////////////////////////////////////////////////////////////////////////////////////////////////
	
	$scope.showFilterBar = function () {
      filterBarInstance = $ionicFilterBar.show({
        items: $scope.items,
        update: function (filteredItems, filterText) {
          $scope.items = filteredItems;
          if (filterText) {
            console.log(filterText);
          }
        }
      });
    };
	
	$scope.searchFullStore = function(){
		var query = $('.filter-bar-search').val();
		if(query.length > 2 )
		{
			$http({method: 'POST',url: urlBase+'/SearchFullStore',data: $.param({Search:query,StoreID:$rootScope.storeId}),headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(
			function successCallback(response) {
				var x2js = new X2JS();
				jsonResponse = x2js.xml_str2json(response.data);
				$scope.products =JSON.parse(jsonResponse.string.__text);
			}
			, function errorCallback(response) {alert(response);});
		}
	};
	
	$("body").on('keyup', '.filter-bar-search', $scope.searchFullStore );	
	
	/////////////////////////////////////////////////////////////////////////////////////////////////
	
	$ionicModal.fromTemplateUrl('templates/saveBuyNSell.html', {
		scope: $scope
	}).then(function(modal) {
		$scope.buyNSellModal = modal;
	});

	$scope.showBuyNSell = function (){
		$scope.buyNSellModal.show();
	};
	
	$scope.closeBuyNSell = function (){
		$scope.buyNSellModal.hide();
	};
	
	$scope.itemPhoto = function (){
		try{
			navigator.camera.getPicture(function(imageURI) {
				$scope.itemImage=imageURI;
			}, 
			function(err) {
			alert('Image failure');
			}, { quality: 10,
			destinationType: Camera.DestinationType.DATA_URL});
		}
		catch(err)
		{
			alert(err);
		}
	};
	
	$scope.saveBuyNSell= function (){
		loadManager.showWaiter();
		$http({method: 'POST',url: urlBase+'/SaveBuyNsell',data: $.param({ID:'0',categoryID: $('#categoryID').val() ,itemname: $('#itemName').val(),locationName:$('#locationName').val(),description:$('#description').val(),sellingPrice:$('#sellingPrice').val(),mrp:$('#mrp').val(),contactnum1:$('#contactnum1').val(),itemage:$('#usage').val(),ImageString:$scope.itemImage}),headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(
		function successCallback(response) {
			var x2js = new X2JS();
			jsonResponse = x2js.xml_str2json(response.data);
			if(jsonResponse.boolean == "true")
			{
				loadManager.hideWaiter();
				$state.go('storeTypes');
				$scope.signUpModal.hide();
				$scope.modal.hide();
				$scope.closeBuyNSell
			}
			else
			{
				loadManager.hideWaiter();
				ionicToast.show('Error In Form.','middle',false,2500);
			}
		}
		, function errorCallback(response) {ionicToast.show('Couldn\'t Connect','middle',false,2500);loadManager.hideWaiter();});
	};
	
	/////////////////////////////////////////////////////////////////////////////////////////////////
	
	/////////////////////////////////////////////////////////////////////////////////////////////////
	
	$ionicModal.fromTemplateUrl('templates/customerBuyNSell.html', {
		scope: $scope
	}).then(function(modal) {
		$scope.customerBuyNSellModal = modal;
	});

	$scope.showCustomerBuyNSell = function (){
		$scope.customerBuyNSellModal.show();
	};
	
	$scope.closeCutomerBuyNSell = function (){
		$scope.customerBuyNSellModal.hide();
	};
	
	$scope.saveBuyNSell= function (){
		loadManager.showWaiter();
		$http({method: 'POST',url: urlBase+'/SaveBuyNsell',data: $.param({ID:'0',categoryID: $('#categoryID').val() ,itemname: $('#itemName').val(),locationName:$('#locationName').val(),description:$('#description').val(),sellingPrice:$('#sellingPrice').val(),mrp:$('#mrp').val(),contactnum1:$('#contactnum1').val(),itemage:$('#usage').val(),ImageString:$scope.itemImage}),headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(
		function successCallback(response) {
			var x2js = new X2JS();
			jsonResponse = x2js.xml_str2json(response.data);
			if(jsonResponse.boolean == "true")
			{
				loadManager.hideWaiter();
				$state.go('storeTypes');
				$scope.signUpModal.hide();
				$scope.modal.hide();
				$scope.closeBuyNSell
			}
			else
			{
				loadManager.hideWaiter();
				ionicToast.show('Error In Form.','middle',false,2500);
			}
		}
		, function errorCallback(response) {ionicToast.show('Couldn\'t Connect','middle',false,2500);loadManager.hideWaiter();});
	};
	
	/////////////////////////////////////////////////////////////////////////////////////////////////
	
	$scope.getOther();
	$scope.getCategoryForStoreType();
	
	$scope.title = 'Services';
});